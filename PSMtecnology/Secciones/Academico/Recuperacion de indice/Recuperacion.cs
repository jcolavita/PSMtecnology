...
namespace TrabajoDeGrado.Secciones.Estudiantes_y_Profesores
{
    public partial class Recuperacion : Form
    {
        ...
        public string usuario;
         ...
         string sql = "SELECT tria.ID, tria.ciestudiante AS CEDULA, tria.titulo AS TITULO, " +
        ...
        private void Btnagregarprofesor_Click(object sender, EventArgs e) 
        {
            ...
            agregartria agregar = new agregartria();
            ...
            panel.Size = new Size(535, 630);
            panel.MinimumSize = new Size(535, 570);
            panel.MaximumSize = new Size(535, 630);
        ...
        {
            string sqlbusqueda = "SELECT tria.ID, tria.ciestudiante AS CEDULA, tria.titulo AS TITULO, " +
            "CONCAT (users.primernombre,' ',users.segundonombre,' ',users.primerapellido,' ', users.segundoapellido) AS ESTUDIANTE," +
            "CONCAT (peri.año,'-',peri.semestre) AS PERIODO, tria.fechadefensa AS DEFENSA, est.escuela AS ESCUELA " +
            "FROM tria tria " +
            "INNER JOIN estudiantes est ON est.ciestudiante = tria.ciestudiante " +
            "INNER JOIN usuarios users ON est.ciestudiante = users.usuario " +
            "INNER JOIN periodo peri ON peri.id = tria.periodo" +
                " WHERE tria.ciestudiante LIKE '%" + tbbusqueda.Text + "%' OR tria.titulo LIKE '%" + tbbusqueda.Text + "%'" +
        ...
        private void Gridprofesores_CellContentClick(object sender, DataGridViewCellEventArgs e) 
        {
            ...
            agregartria modificar = new agregartria();
            if (grid.Columns[e.ColumnIndex].HeaderText == "EDITAR" && e.RowIndex > -1)
            {
                ...
                modificar.lblcabecera.Text = "Modificar Trabajo de Recuperacion de Indice";
       ...
       //filtros para el grid
        ...
        {
            string sql = "";
            string consultabase = "SELECT tria.ID, tria.ciestudiante AS CEDULA, tria.titulo AS TITULO, " +
            "CONCAT (users.primernombre,' ',users.segundonombre,' ',users.primerapellido,' ', users.segundoapellido) AS ESTUDIANTE," +
            "CONCAT (peri.año,'-',peri.semestre) AS PERIODO, tria.fechadefensa AS DEFENSA, est.escuela AS ESCUELA " +
            "FROM tria tria " +
            "INNER JOIN estudiantes est ON est.ciestudiante = tria.ciestudiante " +
        ...
        private void btnreporte_Click(object sender, EventArgs e)
        {
            ...
            selectorreporte.formulario = "tria";
